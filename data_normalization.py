...
def read_from_database_to_list():                               # Take in raw str, and convert to dictionary, then to list
        ...
        bad_companies = ["WLKP", "VMC", "VRS", "REX"]           # These companies data are incomplete. Remove (not enough data)
        bad_days = []
        ...
        counter_i = 0                                           # Input all data to premade 3D list in order
        counter_j = 0
        counter_k = 0
        for day in list_days:
                for company in list_companies:
...
def format_data(data, normalize_function = 2, normalize_company = 1, num_range = 1, batch_size = 16, train_size = 70, val_size = 15, test_size = 15, start_point_diff = 25, start_point_deviation = 5, length = 25, pred_length = 5, company_group = True, random_batch = False, no_change_range = 5):
    ...
    if (data == None):
        ...
        data = read_from_database_to_list()
